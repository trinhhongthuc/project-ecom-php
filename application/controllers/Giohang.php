...
class Giohang extends CI_Controller {
    ...
    public function info_order(){
        ...
        $array_items = array();
        $data2=$this->session->userdata('cart');
        $price = 0;
        $number_index = 0;
        foreach ($data2 as $key => $value) {
            $row = $this->Mproduct->product_detail_id($key);
            $array_items[] = $row['shop_name'];
            // $price = $price + $row['shop_name']
         
        }
        $data3=$this->session->userdata('cart');
        $array_products_name = array();
        foreach ($data3 as $key => $value) {
            $row = $this->Mproduct->product_detail_id($key);
            $array_products_name[] = $row['name'];
        ...
        // foreach ($array_items as $value) {
            if($this->form_validation->run() == TRUE){
                ...
                if(isset($_POST['price_vocher'])) {
                    $vorcher_price = $_POST['price_vocher'];
                }else {
                    $vorcher_price = 0;
                }
                ...
                foreach ($array_items as $key => $value) {
                    $coderesult = random_string('alnum', 8);

                    if($vorcher_price != 0 && $number_vocher == 1 ) {
                        $mydata=array(
                            'orderCode' => $coderesult,
                            ...
                            'coupon' => $vorcher_price,
                            'province' => $provinceId,
                            'district' => $districtId,
                            'trash' => 1,
                            'status' => 0,
                            'shop_name'=>$value,
                            'name_product'=>$array_products_name[$index],
                            // 'price_product'=>
                        );
                        $number_vocher++;
                    } else {
                ...
                $this->data['title']='Sàn thương mại điện tử - Thông tin đơn hàng ';
    ...
    public function thankyou(){
        if($this->session->userdata('info-customer')||$this->session->userdata('sessionKhachHang')){
            ...
            $config['smtp_pass']    = '0935945052'; //mk gmail
            ...
            $this->email->from('trinhhongthuc57@gmail.com', 'Smart Store');
            $list = array($val['email']);
            $this->email->to($list);
            $this->email->subject('Sàn thương mại điện tử');
        ...
        $this->data['title']='Sàn thương mại điện tử';
