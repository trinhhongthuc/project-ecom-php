...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->data['title']='Sàn thương mại điện tử - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        $this->form_validation->set_rules('username', 'Tên đăng nhập', 'required|min_length[6]|max_length[32]|is_unique[db_user.username]');
        ...
        $this->form_validation->set_rules('shop_name', 'Tên shop', 'is_unique[db_user.shop_name]');
        if($this->form_validation->run() ==TRUE){

            if( $this->input->post('phanquyen') == '' || $this->input->post('phanquyen') == 'khachhang' ){
                $phanquyen ="khachhang";
            }else {
            ...
            if($this->input->post('shop_name') == '') {
                ...
                $nameshope = $this->input->post('shop_name');
            }
            ...
            if( $phanquyen == "khachhang" ){
                ...
                $this->Mcustomer->customer_insert_customer($data1);
            }else  {
                    $this->Mcustomer->customer_insert_user($data);
            ...
            $config['smtp_user']    = 'trinhhongthuc57@gmail.com';
            ...
            // $this->email->from('trinhhongthuc57@gmail.com', 'Sàn thương mại điện tử');
            // $this->email->to($email);
            // $this->email->subject('Hệ thống Sàn thương mại điện tử - Quà thành viên mới');
            // $this->email->message(' Bạn đã trở thành thành viên mới của cửa hàng Sàn thương mại điện tử, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : '.$tempcoupon.' , Mã này có giá trị tới ngày '.$tempdatelimit.'
            //     Hãy sử dụng tài khoản để mua hàng để tích lũy nhận thêm nhiều ưu đãi !!!!');
            // $this->email->send();
            $this->data['success']='Đăng ký thành công! Vui lòng đăng nhập để mua các sản phẩm của chúng tôi!!';
            echo '<script>alert("Đăng ký tài khoản thành công! Vui lòng đăng nhập để bắt đầu mua sắm")</script>';
            echo '<script>window.location.href = "http://localhost/webapp/dang-nhap";</script>';

        }  
        $this->data['title']='Sàn thương mại điện tử - Đăng ký tài khoản';   
    ...
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_port']    = '465';
            $config['smtp_timeout'] = '7';
            $config['smtp_user']    = 'trinhhongthuc@gmail.com';
            $config['smtp_pass']    = 'cqfmfmrtudhcmahw';
            $config['charset']    = 'utf-8';
            $config['newline']    = "\r\n";
            $config['wordwrap'] = TRUE;
            ...
            $config['validation'] = TRUE;   
            // $this->email->initialize($config);
            // $this->email->from('trinhhongthuc@gmail.com', 'Sàn thương mại điện tử');
            // $this->email->to($list['email']);
            // $this->email->subject('Hệ thống Sàn thương mại điện tử - Lấy lại mật khẩu');
        ...
        $this->data['title']='Sàn thương mại điện tử - Quên mật khẩu';   
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='Sàn thương mại điện tử - Cập nhật mật khẩu mới';
